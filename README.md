...
- 부트스트랩(Booststrap)
    ...
    - Raw Data의 본포를 추정할 때 사용 ( 측정된 데이터 중에서 중복을 허용한 복원 추출로 n개를 뽑고, 뽑은 n개의 평균을 구하는 것을 m번 반복하여 모아 평균에 대한 분포 구하기, sampling mean에 대한 신뢰구간을 추정)
    - 머신러닝에서는 Random Sampling을 통해 training data를 늘릴 수 있음
    - 배깅(Bagging)은 부트스트랩(bootstrap)을 집계(Aggregating)하여 학습 데이터가 충분하지 않더라도 충분한 학습효과를 주어 높은 bias의 underfitting 문제나, 높은 variance로 인한 overfitting 문제를 해결하는데 도움
...
- 배깅(Bagging; Bootstrap Aggregating)
    ...
    - 각각의 Weak Tree를 균등한 비중으로 앙상블 하여 최종 결과를 도출
...
- 일반적인 의사결정나무는 한층에서 가지를 만들면 다른 층에서 가지를 만들어주고 다음 단계로 넘어가는데, LightGBM은 층에 제약없이 잎에서 가지를 치고 또 가지를 치는 방식으로 의사결정나무 생성
...
### 3.4 AutoML
- AutoML이란 Automated Machine Learning의 약자, 머신러닝 모델 개발을 자동화하는 분야.
- 많은 머신러닝 모델 개발 과제에서 반복적으로 수행하는 되는 데이터 분할, 정제, 특징 선택 및 추출, 모델 선택, 하이퍼 파라미터 튜닝 등을 함수화하여 자동화 하는 것.

<p align="center">
    <img src="images/automl.PNG">
</p>

- 그러나 하이퍼 파라미터의 종류가 많아질수록 탐색 시간이 기하급수적으로 증가하고 작업을 병렬화 할 수 없다는 단점 때문에 경제성이 떨어지는 경향이 있음 

### 3.5 Ensemble
- 여러 단일 예측(or 분류)모델을 하나로 엮어 더 좋은 성능의 복합 모델을 만드는 기법을 말함.
- 가장 많이 쓰이는 유형은 Voting, Bagging(Random Forest), Boosting(AdaBoost, Gradient Boost, XGBoost, LGBM)등이 있음.

- Voting 기법은 앙상블 기법 중 가장 간단한 기법으로 분류(Classification) 문제에서 활용.
- 각각의 단일 모델들이 예측한 분류 중 가장 많은 비율을 차지한 레이블을 최종 결과로 예측.

- Bagging 기법은 Bootstrapping + Aggregating의 합성 용어
- 전체 집합에서 무작위 복원추출을 통해 여러 부분집합을 만드는 행위
- Bagging에서 Aggregating이란 Bootstrapping을 통해 생성된 각기 다른 데이터셋으로부터 학습한 여러 모델들의 아웃풋을 집계하여 최종 예측값/분류값을 도출하는 과정

- Boosting은 Bagging과 다르게 앙상블 되는 모델은 동일한 알고리즘 기반 모델이며, 각 모델들이 병렬적이 아닌 순차적으로 학습하고, 집계 시 각 모델의 출력에 가중치를 적용하여 집계 
...
## 5. 프로젝트 결과
- 대용량 데이터를 처리하고 EDA 하는데 상당한 시간과 메모리 용량이 필요하다.
- 데이터의 종류에 따른 차별화 된 결측치 처리 방식이 성능을 향상시켰다.
- train 데이터의 샘플수와 label의 샘플수를 동일하게 처리해주기 위해 중복되는 데이터를 통합하는 방향으로 전처리를 진행했을때 좋은 성능을 보였다.  
- 단일 모델의 성능 향상에는 한계가 존재함.
...
- 316 / 4874의 성적을 거두었고, 상위 6%의 성적을 기록한 덕분에 kaggle에서 동메달을 수상하였습니다.
